...
class MoneyGenerator():
    ...
    def handle_event(self, pygame, event, player_funds):
        temp = {'flag':None, 'cost': 0}  # Temp[0] = -1 if no button is clicked or button has no function
        if self.rect.collidepoint(event.pos):
            for button in self.buttons:
                button_id = button.handle_event(pygame, event)
                if button_id != -1:
                    # if not enough money to purchase money gen
                    temp['flag'] = 'insufficient'
                    if button_id == 2:
                        temp['cost'] = self.get_cost(1)
                        # if you have enough funds
                        if player_funds >= temp['cost']:
                            self.level_up(1)
                            temp['flag'] = 'sufficient'
                            # return temp
                        return temp
                    elif button_id == 3:
                        temp['cost'] = self.get_cost(10)
                        if player_funds >= temp['cost']:
                            self.level_up(10)
                            temp['flag'] = 'sufficient'
                            # return temp
    ...
    def hover_check(self, mouse_pos):
        upgrade_cost = False # Check the next money generator
        if self.rect.collidepoint(mouse_pos):
            for button in self.buttons:
                button_id = button.hover_check(mouse_pos)
                if button_id is not False:
                    if button_id == 2:
                        upgrade_cost = self.get_cost(1)
                    elif button_id == 3:
                        upgrade_cost = self.get_cost(10)
                    elif button_id == 4:
                        upgrade_cost = self.get_cost(100)
                    return upgrade_cost
        return upgrade_cost # Check the next money generator
